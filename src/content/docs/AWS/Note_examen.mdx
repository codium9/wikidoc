...
| Service         | Protocole | OS Cible           | Points forts pour l'examen                          |
| :-------------- | :-------- | :----------------- | :-------------------------------------------------- |
| Amazon EFS      | NFS v4    | Linux uniquement   | Serverless, scaling automatique, POSIX.             |
| FSx for Windows | SMB       | Windows (et Linux) | Active Directory, NTFS ACLs, SMB.                   |
| FSx for Lustre  | POSIX     | Linux (High Perf)  | Très haut débit (HPC, Machine Learning).            |
| S3 Mountpoint   | API S3    | Tous               | Pas un "vrai" système de fichiers (pas de verrous). |
