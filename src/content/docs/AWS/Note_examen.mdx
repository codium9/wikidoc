...
| Caractéristique      | Amazon CloudFront                  | AWS Global Accelerator             |
| :------------------- | :--------------------------------- | :--------------------------------- |
| Objectif principal   | Mise en cache de contenu (CDN)     | Optimisation réseau (Routage)      |
| Mecanisme principal  | Mise en cache (Edge Caching)       | Routage optimise via IP Anycast    |
| Couche OSI           | Couche 7 (HTTP / HTTPS)            | Couches 3 & 4 (TCP / UDP)          |
| Type d'IP            | IP dynamiques (via DNS)            | 2 IP Statiques Anycast             |
| Protocoles supportés | HTTP, HTTPS, WebSockets            | TCP, UDP, HTTP, HTTPS              |
| Cas d'usage typique  | Vidéo, Images, Sites Web statiques | Gaming, VoIP, IoT, Apps non-HTTP   |
| Failover (Panne)     | Dépend du TTL DNS (plus lent)      | Instantané (l'IP ne change jamais) |
| Sécurité             | Intégration native avec WAF        | Protection Shield (DDoS)           |


#### **Fonctionnalités avancées de CloudFront**

| Fonctionnalité CloudFront | Usage                               | Bénéfice Examen                                          |
| :------------------------ | :---------------------------------- | :------------------------------------------------------- |
| Origin Groups             | Haute disponibilité (Failover)      | Bascule auto vers un S3 secondaire si le primaire echoue |
| Content Type Routing      | Routage par comportement (Behavior) | Envoie les images vers S3 et le dynamique vers l ALB     |
| Field-level encryption    | Chiffrement de champs specifiques   | Protege les donnees sensibles de bout en bout (PII/PCI)  |
| Geo Restriction           | Blocage de pays                     | Empeche l acces selon la localisation géographique       |

#### **Typde de certificat TLS sur un load balancer**

| Type de certificat  | Usage                                                  | Limite majeure                                         |
| :------------------ | :----------------------------------------------------- | :----------------------------------------------------- |
| Certificat Unique   | Un seul nom de domaine (ex: mycorp.com)                | Ne gere pas les sous-domaines ou domaines tiers        |
| Certificat Wildcard | Un domaine et ses sous-domaines directs (*.mycorp.com) | Incompatible avec un second domaine (ex: yourcorp.com) |
| Certificat SAN      | Plusieurs noms specifiques dans un seul certificat     | Lourd a mettre a jour car il faut regenerer le fichier |
| SNI                 | Plusieurs certificats distincts sur un seul ALB        | Solution native pour le multi-domaine et microservices |
...
#### **Storage Gateway - Comparatif des modes**

| Mode Gateway            | Stockage Primaire      | Cache Local           | Cas d usage ideal                           |
| :---------------------- | :--------------------- | :-------------------- | :------------------------------------------ |
| Volume Gateway (Cached) | Amazon S3              | Oui (donnees actives) | Extension de stockage local illimitee       |
| Volume Gateway (Stored) | Local (On-premises)    | Non (tout est local)  | Backup et Disaster Recovery                 |
| S3 File Gateway         | Amazon S3              | Oui                   | Acces NFS/SMB aux fichiers S3               |
| Amazon FSx File Gateway | Amazon FSx for Windows | Oui                   | Performance native Windows/AD dans le cloud |
...
#### **Cas d'usage des transferts S3**

| Methode de transfert     | Cas d usage                         | Seuil critique                |
| :----------------------- | :---------------------------------- | :---------------------------- |
| Single Put               | Petits fichiers                     | Max 5 Go                      |
| Multi-part Upload        | Gros fichiers (> 100 Mo recommende) | Obligatoire > 5 Go            |
| S3 Transfer Acceleration | Longue distance (intercontinental)  | Utilise les Edge Locations    |
| Direct Connect           | Connexion privee reseau permanente  | Pas specifique a un upload S3 |
| Snowball                 | Petabytes de donnees (hors ligne)   | Quand le reseau est trop lent |
...
| Type de Politique    | Attache a quoi               | Fonction principale                              |
| :------------------- | :--------------------------- | :----------------------------------------------- |
| Identity-based       | Utilisateur, Groupe, Role    | Definit ce que l identite peut faire             |
| Resource-based       | S3, SNS, Role (Trust Policy) | Definit qui peut acceder a cette ressource       |
| Trust Policy         | Role IAM                     | Definit qui est autorise a endosser le Role      |
| Permissions Boundary | Utilisateur, Role            | Definit le plafond maximum de droits             |
| SCP (Organizations)  | Compte, OU                   | Limite les actions possibles au niveau du compte |
...
#### DynamoDB - Type de table

| Caractéristique | Table Standard                      | Global Table                                  |
| :-------------- | :---------------------------------- | :-------------------------------------------- |
| Portée          | Une seule région AWS                | Multiples régions AWS                         |
| Cas d usage     | Application locale ou régionale     | Application mondiale / Disaster Recovery      |
| Réplication     | Automatique sur 3 AZ                | Automatique entre plusieurs régions           |
| Complexité      | Faible                              | Élevée (gestion des conflits d ecriture)      |
| Utile si        | Traffic variable et pic d'activitée | Latence faible ou restauration suite désastre |

#### Elasticache VS DynamoBD

| Caracteristique   | ElastiCache                         | DynamoDB                               |
| :---------------- | :---------------------------------- | :------------------------------------- |
| Support physique  | Memoire vive (RAM)                  | Disque SSD                             |
| Latence type      | Microsecondes                       | Millisecondes                          |
| Mot-cle examen    | In-memory cache-based               | Serverless, Key-value store            |
| Gestion du TTL    | Immediate et precise                | Jusqu a 48h pour suppression reelle    |
| Persistance       | Perte possible si crash sans backup | Garantie (ecriture sur disque)         |
| Cas d usage ideal | Performance extreme, sessions web   | Simplicite, cout a l usage, Serverless |
...
##### Service PaaS - Comparatif

| Service               | Cas d usage ideal                                | Mot-cle Examen                             |
| :-------------------- | :----------------------------------------------- | :----------------------------------------- |
| AWS Amplify           | Applications mobiles et web modernes (React/Vue) | Full-stack, Mobile, Serverless backend     |
| AWS Elastic Beanstalk | Applications web classiques (Java, PHP, .NET)    | PaaS, Auto Scaling, Full control EC2       |
| Amazon Lightsail      | Petits projets, blogs ou environnements de test  | VPS, Predictable monthly price, Simplicity |