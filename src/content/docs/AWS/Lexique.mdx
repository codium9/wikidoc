...
### 🌐 **Réseau (Networking)**
	...
	- API Gateway : Point d'entrée sécurisé pour exposer et gérer des API à grande échelle. (caching API possible pour données peu rafraîchies)
	- Direct Connect : Connexion réseau privée et dédiée entre votre centre de données et AWS.
	- VPC Peering : Connexion privée entre deux VPC pour un trafic sécurisé et à faible latence. (valable pour peu de vpc / même région et account)
...
### 📨 **Intégration et Migration**
	...
	- DMS (Database Migration Service) : Moteur de réplication de données en temps réel - principalement utilisé pour de la migration BDD, mais pas que.
	...
	- MQ : Service de courtage de messages compatible avec les protocoles standard (AMQP, MQTT, STOMP).
...
### 🔎 **Gestion et Monitoring**
	...
	- Compute Optimizer : Recommandations basées sur les métrics cloudwatch (CPU/RAM) pour optimiser les ressources de calcul (EC2, Lambda, EBS).
